...
Ported from nemotron-tonghuikang-source/augmenters/concatenation.py with
no logic changes -- record shape is normalised to our local convention
...
Pure RNG over 28 ASCII punctuation symbols (same set used by cryptarithm).
Each generated problem is one prompt + one completion of 100 lines.
"""
...
# THK defaults. Keep aligned so per-category counts match their corpus.jsonl.
N_PROBLEMS = 1500
...
def generate(n_problems: int = N_PROBLEMS, seed: int = 99) -> list[dict[str, str]]:
    """Generate concatenation problems.

    Args:
        n_problems: Override the default count (useful for smoke tests).
        seed: RNG seed for reproducibility. THK uses 99 for this augmenter.

    Returns: list of {id, prompt, completion, category} dicts.
    """
