...
### 2026-05-04 — First scored LoRA iterations; truncation identified as the dominant ceiling; iteration 3 with stratified data queued

**End-of-day status:** Standard LoRA training is done end-to-end and scored — two paired evals complete (iterations 1 and 2). Iteration 2 reaches the published Nemotron-3-Nano AIME25 number (~83%) on the **subset of problems where the model finishes reasoning**, but Kaggle's 7680-token cap interrupts the model mid-reasoning on hard problems and zeros them out. So the headline gap from ~33% raw accuracy to the published ~85% is a *truncation* ceiling under Kaggle's eval params, not a reasoning-capacity ceiling. Iteration 3 (mixed short + medium length training data, queued) is the response. Infra hardened on the side: train walltime raised, live step logs enabled, eval can now route to H100 or H200 to cut queue wait.

#### 1. Standard LoRA is working — but the budget cap binds on complex problems

- **Iteration 1 (jobs 6525101 / 6533534): null result.** LoRA 30.0% vs base 33.3% — within 1-problem noise. Root cause was data-pipeline truncation: [scripts/train_lora.py](scripts/train_lora.py) cut every tokenized trace at `max_seq_len=4096`, slicing the closing `\boxed{...}` off the supervision target for most OpenMathReasoning samples (which run 5-15K tokens). Loss was flat at ~0.30 across all 187 optim steps with no descent. Fixed by lowering `--max-trace-chars` from 14000 → 10000 in data prep, raising `max_seq_len` from 4096 → 8192 in the training config, and biasing toward the shortest of a 2× candidate pool.
- **Iteration 2 (jobs 6540448 / 6547439): real signal.** LoRA 33.3% (10/30) vs base 26.7% (8/30) — base accuracy moves run-to-run because vLLM at greedy `temperature=0.0` is not bit-exact deterministic across runs. Mean LoRA response length dropped 21% vs base. **Critical finding: when LoRA does NOT truncate, accuracy is 10/12 = 83%** — essentially at the published Nemotron-3-Nano AIME25 number. Stop rate (model emits EOS with `\boxed{}` naturally) doubled vs base (12/30 vs 6/30); the 18/30 problems where LoRA still hits the 7680-token cap are the genuinely hard problems requiring 3-5K tokens of reasoning.
- **Iteration 3 (queued): stratified mixed-length data.** Training set is now 1500 short traces (3-6K chars / ~1-2K tokens) plus 1500 medium traces (9-15K chars / ~3-5K tokens), all complete with `\boxed{}`. Theory: teach the LoRA that *both* length regimes are valid completions, so hard problems get more reasoning budget while easy problems still finish quickly. Single-axis change vs iteration 2 — data composition only; same rank=16, LR=2e-4 cosine, 1 epoch. Detail in [train_strategies.txt](train_strategies.txt) "Iteration 3" section.

#### 2. Infra hardening — walltime, live logs, finish_reason capture, H100/H200 eval routing

- **`slurm/train.slurm` walltime bumped 4h → 5h.** Iteration 2 used 3h6m of 4h walltime; the margin was thin and [scripts/train_lora.py](scripts/train_lora.py) only saves the adapter at the very end (no intermediate checkpoints), so a walltime overrun = zero artifact for 4 GPU-hours. Cheap insurance against the next iteration's tighter scheduling.
- **`slurm/train.slurm` now exports `PYTHONUNBUFFERED=1`.** Iteration 2 ran 3h with no visible step logs because Python's default block buffering held all stdout in a buffer until process exit. Live `tail -f` step logs now enable early-abort of a non-learning run instead of waiting the walltime out.
- **Eval phase routes to H100 or H200 via CLI override** — no duplicate Slurm script needed, just override `--gres` at submission:
  ```bash
  sbatch slurm/eval.slurm                       # H200 (default)
  sbatch --gres=gpu:h100:1 slurm/eval.slurm     # H100
  ```
  Submitting both and scancel'ing the laggard cuts queue wait when the H200 partition is busy. The 30B BF16 model fits on H100 (60 GB model + ~8 GB headroom at `gpu_memory_utilization=0.85`); bump to 0.92 in [configs/eval_kaggle.yaml](configs/eval_kaggle.yaml) if H100 OOMs.
- **[scripts/baseline_generate.py](scripts/baseline_generate.py) now records `finish_reason` and `num_response_tokens`.** Iteration 1's truncation diagnosis required back-tokenizing the JSONL after the fact; iteration 2's at_cap/stop accounting was a one-liner because the fields are stored directly. Eliminates a recurring diagnostic blindspot for all future evals.
- **[scripts/prepare_reasoning_traces.py](scripts/prepare_reasoning_traces.py) gained `--min-trace-chars`, `--shortest-of`, and 5xx retry handling.** Lower-bound filter enables stratified bin construction; shortest-of biases the candidate pool toward concise traces; the HF datasets-server returned a 502 mid-scrape today and the previous handler only retried 429, so the run crashed after collecting 1076/3000 candidates — now any 5xx response is retried with the same exponential backoff as 429.
...
Three problems surfaced during today's diagnostic round (train job 6664554 → 0.57 on Kaggle; eval jobs 6673063 / 6673562 stratified-holdout timeouts).

#### 1. Long reasoning traces (cipher, bit_manipulation) don't translate into model ability

`data/sft_traces.jsonl` already includes ~1380 cipher rows (assistant p50 ≈ 330 tokens) and ~1190 bit_manipulation rows (assistant p50 ≈ 2200 tokens). The bit_manipulation traces were verified mathematically correct against the example mappings — they brute-force enumerate ~50 candidate operations × 8 output bits, validate against 10 examples, select the unique winner per bit, then apply. Despite this, three Kaggle submissions trained on the same data plateaued at 0.53 / 0.56 / 0.57. Decomposing the leaderboard score as `(3 × 100% + 38% + 4% + 0%) / 6 ≈ 0.57` reproduces the number to two decimal places, implying bit_manipulation accuracy on Kaggle is in the single digits. The likely mechanism: a 2200-token tabular brute-force procedure asks a rank-16 LoRA on a 30B base to execute exact 8-bit arithmetic over hundreds of intermediate steps, and arithmetic drift across the long horizon makes the boxed answer wrong. The traces teach the *format* of bit-op reasoning, not the *correctness*. The same may hold for the harder cipher cases (under-determined letters fall back to guessing) — local accuracy ~38% suggests the trace style is partly correct but doesn't generalize beyond seen examples.

#### 2. Current eval script carries a real timewall risk

`slurm/eval.slurm` runs Stage 3 (base eval) + Stage 4 (LoRA eval) sequentially in a single SLURM allocation, each loading the 30B model fresh through vLLM. With the 200-prompt single-category `numeral_holdout.jsonl` this fit comfortably in 1h (job 6672000 finished in 14:42). Switching to the 600-prompt stratified holdout pushed the same script over the 1h walltime (job 6673063 → `TIMEOUT`); after bumping walltime to 2:30, job 6673562 still ran ≥1h42m without producing Stage 5 output. The cost driver is bit_manipulation / equation_numeric prompts — at `temp=0.0, max_tokens=7680` the *base* model has no training signal to emit `\boxed{}` early and can generate to the cap on every prompt. Two passes over 600 mixed-category prompts at this latency profile is genuinely expensive, and the script provides no per-prompt progress logging (vLLM async + `disable_log_stats=True` means the .out file sits at `Final IR op priority` for the entire silent inference window — looks identical to a hang). Running diagnostics through this pipeline is becoming the bottleneck, not just the training; until eval is split (base vs LoRA in separate jobs, or smaller holdout for fast feedback), every diagnostic round costs >2h of wallclock.
